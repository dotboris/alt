...
fn success_with_no_problems() -> IoResult<()> {
    ...
    let bin_v1_path = env.create_stub_command("thingy-1", "this is thingy v1")?;
    let bin_v2_path = env.create_stub_command("thingy-2", "this is thingy v2")?;
...
fn remove_entry_for_missing_bin() -> IoResult<()> {
    ...
    env.alt().arg("show").assert().success().stdout(
...
fn remove_entry_for_non_executable_bin() -> IoResult<()> {
    ...
    let mut perm = fs::metadata(&bin_v1_path)?.permissions();
    perm.set_mode(0o644);
